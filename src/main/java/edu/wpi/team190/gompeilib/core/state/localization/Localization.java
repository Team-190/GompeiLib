...
public class Localization {
    ...
    public Localization(
        ...
        this.estimationRegions = estimationZones.stream()
                .map(zone -> new EstimationRegion(zone.aprilTags(), kinematics))
                .toList();
    }

    public void addOdometryObservation(double timestamp, Rotation2d rawHeading, SwerveModulePosition[] modulePositions) {
      swerveDriveOdometry.update(rawHeading, modulePositions);
      poseBuffer.addSample(timestamp, swerveDriveOdometry.getPoseMeters());
      estimationRegions.forEach(region -> region.addOdometryObservation(timestamp, rawHeading, modulePositions));
    }
