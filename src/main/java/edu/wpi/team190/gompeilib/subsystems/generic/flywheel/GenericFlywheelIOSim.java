...
public class GenericFlywheelIOSim extends GenericFlywheelIO {
  ...
  public GenericFlywheelIOSim(GenericFlywheelConstants constants) {
    motorSim =
        new FlywheelSim(
            LinearSystemId.createFlywheelSystem(constants.GENERIC_FLYWHEEL_PARAMETERS.MOTOR_CONFIG(), constants.MOMENT_OF_INERTIA, constants.GEAR_RATIO),
    ...
    feedforward =
        new SimpleMotorFeedforward(
            constants.GAINS.kS().get(), constants.GAINS.kV().get());
  ...
  public void updateInputs(GenericFlywheelIOInputs inputs) {
    ...
    inputs.positionRadians = Rotation2d.fromRadians(motorSim.getAngularPositionRad());
