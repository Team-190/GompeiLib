...
public class SwerveModuleIOTalonFXSimTest {
  ...
  public void setUp() {
    ...
    DCMotor driveModel = DCMotor.getNeo550(1);
    DCMotor turnModel = DCMotor.getNeo550(1);
  ...
  public void testTalonFXSimIO() {
    ...
    TalonFXSimState driveSimState = mock(TalonFXSimState.class);
    TalonFXSimState turnSimState = mock(TalonFXSimState.class);
    CANcoderSimState cancoderSimState = mock(CANcoderSimState.class);

    when(driveSimState.getMotorVoltage()).thenReturn(6.0);
    when(turnSimState.getMotorVoltage()).thenReturn(4.0);
    ...
    try (MockedConstruction<TalonFX> mockTalon =
            mockConstruction(
                ...
                (mock, context) -> {
                  ...
                  if (context.getCount() == 1) {
                    ...
                    when(mock.getSimState()).thenReturn(driveSimState);
                  } else {
                    ...
                    when(mock.getSimState()).thenReturn(turnSimState);
                  }
        ...
        MockedConstruction<CANcoder> mockCANcoder =
            mockConstruction(
                ...
                (mock, context) -> {
                  ...
                  when(mock.getSimState()).thenReturn(cancoderSimState);
                });
      ...
      // Now construct the sim IO object
      SwerveModuleIOTalonFXSim sim = new SwerveModuleIOTalonFXSim(driveConstants, moduleConstants);

      TalonFX driveTalon = mockTalon.constructed().get(0);
      TalonFX turnTalon = mockTalon.constructed().get(1);

      when(driveTalon.getSimState()).thenReturn(driveSimState);
      when(turnTalon.getSimState()).thenReturn(turnSimState);

      // update stubbing to return the correct signals per motor
      ...
      // Verify that getMotorVoltage() was called and setters on SimStates were invoked
      verify(driveSimState).setSupplyVoltage(anyDouble());
      verify(driveSimState).setRawRotorPosition(anyDouble());
      verify(driveSimState).setRotorVelocity(anyDouble());

      verify(turnSimState).setSupplyVoltage(anyDouble());
      verify(turnSimState).setRawRotorPosition(anyDouble());
      verify(turnSimState).setRotorVelocity(anyDouble());

      verify(cancoderSimState).setRawPosition(anyDouble());
    }
  }
}
